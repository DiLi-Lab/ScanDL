...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        alphas = 1.0 - betas  # shape [diffusion_steps]
        # cumulative product of elements; if alphas = [a1, a2, ..., a_n], self.alphas_cumprod = [a1, a1*a2, a1*a2*a3,..]
    ...
    def p_sample_loop(
        ...
        :param noise: if specified, the noise from the encoder to sample.
                      the sn-sp embedded input, where the sp was replaced by noise.
                      Should be of the same shape as `shape`. the embedded input where the sp was completely
                      replaced by noise.
        :param pos_enc: the positional embeddings
        :param mask_sn_padding:
        :param mask_transformer_att: the attention mask for the transformer
    ...
    def p_sample_loop_progressive(
        ...
        # the number of diffusion steps in reverse order; for 2000 [1999, 1998, 1997, ..., 0]
    ...
    def _token_discrete_loss(self, x_t, get_logits, input_ids, mask=None, truncate=False, t=None):
        ...
        '''
        reshaped_x_t = x_t
        logits = get_logits(reshaped_x_t)  # of shape [microbatch size, seq_len, vocabulary]
        # print(logits.shape)
    ...
    def training_losses_seq2seq(
        ...
        :param sp_word_ids: the word IDs of the scanpath only.
        ...
        microbatch_size, seq_len = sp_word_ids.shape

        # get the word ID embedding and positional embedding (no BERT embedding bc no sentence)
        sn_sp_emb, pos_enc = model.model.module.get_embeds(
        ...
        # map the word ID emb to x_start, which is a one-step noised sn_sp_emb (in paper it's z_0)
        ...
        decoder_nll = self._token_discrete_loss(x_start, get_logits, sp_word_ids)
